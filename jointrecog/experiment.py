...
from jointrecog.datasets import get_dataset
from jointrecog.models import get_model
from jointrecog.utils.stdout_capturing import capture_outputs
from jointrecog.settings import EXPER_PATH, DATA_PATH
...
def train(config, n_iter, output_dir, checkpoint_name='model.ckpt'):
    ...
    with _init_graph(config) as net:
        if 'weights' in config:
            net.load(os.path.join(DATA_PATH, config['weights']))
        try:
